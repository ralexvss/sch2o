...
def toma_lectura_list(request):
    ...
    template_name = 'catastro/lectura_toma.html'
    queryset = Catastro.objects.filter(
        suspender=False).order_by('abonado__apellidos')
...
def lecturaFaltanIngresarList(request):

    template_name = 'catastro/lectura_faltan_ingresar_modal.html'
    context = {}
    faltan_ingresar = LecturaDetalle.objects.raw(
        "SELECT abonado_id, numero FROM catastro_catastro WHERE id NOT IN(SELECT catastro_id FROM catastro_lecturadetalle WHERE lectura_id=7)")
    #raw("SELECT abonado_id, numero FROM `catastro_catastro` WHERE `id` NOT IN(SELECT `catastro_id` FROM `catastro_lecturadetalle` WHERE `lectura_id`=7)")
    context = {
        'faltan_ingresar': faltan_ingresar,
...
class LecturaList(SinPrivilegios, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = Lectura.objects.all().order_by('-id')
        # devuelvo el resultado o la lista resultado

        #queryset = super(LibroAutorListView, self).get_queryset()
        # queryset = queryset # TODO
        return queryset

    def get_context_data(self, **kwargs):
        context = super(LecturaList, self).get_context_data(**kwargs)
        context['ahora'] = timezone.datetime.now()
        return context


def lecturaEmitidasList(request, id_lectura=None):
    lectura = Lectura.objects.all().order_by('-id')
    #lectura_detalle = LecturaDelete.objects.filter(pk=1)
    contexto = {}
    template_name = 'catastro/lectura_emitidas.html'
    contexto = {
        'lectura': lectura,
        'ahora': timezone.now(),
    }
...
def lecturaDetalleEmitidasList(request, lectura_id=None):
    lectura_detalle = LecturaDetalle.objects.filter(lectura_id=lectura_id)
    total_catastro = Catastro.objects.filter(estado=True).count()
    total_lecturas = LecturaDetalle.objects.filter(
        lectura_id=lectura_id).count()
    falta_ingresar = 0
    contexto = {}
    template_name = 'catastro/lecturadetalle_emitidas.html'
    contexto = {
        'lectura_detalle': lectura_detalle,
        'total_catastro': total_catastro,
        'total_lecturas': total_lecturas,
        'falta_ingresar': int(total_catastro) - int(total_lecturas),
...
def lecturaFaltaIngresar(request, lectura_id=None):
    faltan_ingresar = LecturaDetalle.objects.all()
    lectura = request.POST.get("+lectura")
    #model = Catastro.objects.raw('SELECT catastro_catastro.id, catastro_catastro.abonado_id FROM catastro_catastro WHERE id NOT IN ( SELECT catastro_id FROM catastro_lecturadetalle WHERE lectura_id=%s)', [lectura_id])
    #model = list(LecturaDetalle.objects.select_related('catastro').filter(lectura=5))
    contexto = {}
    template_name = 'catastro/lectura_falta_ingresar.html'
    contexto = {
        'model': faltan_ingresar,
    }
    return render(request,  template_name, contexto)
...
def lectura(request, lectura_id=None):
    ...
    for lectura_detalle in LecturaDetalle.objects.all().select_related('catastro').filter(lectura_id=5):
        l = l + 1
        print(l, "Catastro no ingresado", lectura_detalle)
        ...
        if int(consumo) > catas.servicio.base_consumo:
            ...
            for tar in tarifas:
                if int(consumo) >= tar.rango_inicial and int(consumo) <= tar.rango_superior:
                    ...
                    valor_consumo_maximo = 0
                    metros_excedidos = int(consumo) - \
                        int(catas.servicio.base_consumo)
                    ...
                    tarif = Tarifa.objects.filter(
                        rango_superior=maximo['rango_superior__max']).first()
                    tarifa = tarif
