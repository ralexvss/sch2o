...
def informe_caja_global(request):
    ...
    results = Recaudacion.objects.filter(
        fecha__range=(fecha_inicial, fecha_final)).distinct()
    subtotal = Recaudacion.objects.filter(fecha__range=(
        fecha_inicial, fecha_final)).distinct().aggregate(Sum('subtotal'))
...
class RecaudacionList(ListView):
...
class ConsultarPlanilla(ListView):

    #permission_required = 'catastro.view_lecturadetalle'
    model = LecturaDetalle
    template_name = "recaudacion/consultar_planilla.html"
    queryset = LecturaDetalle.objects.filter(estado=True)

    def get_context_data(self, **kwargs):
        context = super(ConsultarPlanilla, self).get_context_data(**kwargs)
        # context.update({'ahora':self.ahora}) #tambien funciona
        context['ahora'] = timezone.now()
        context['entidad'] = Entidad.objects.all().first()
        return context
...
def recaudacion(request, recaudacion_id=None, catastro_id=None):
    ...
    queryset = Recaudacion.objects.all().last()
    ultimo = queryset.id + 1
    template_name = 'recaudacion/recaudacion_create.html'
    cat = Catastro.objects.filter(
        estado=True, suspender=False, pk=catastro_id).first()
    today = timezone.now()
    ...
    if request.method == 'GET':
        ...
        lectura_detalle = LecturaDetalle.objects.filter(
            estado=True, catastro=catastro_id).order_by('-id')
        ...
        subtotal = total_base + total_base_reserva + total_excedente + total_consumo_maximo + \
            total_administracion + total_alcantarillado + \
            total_derecho_conexion + total_otros
        valor = cat.descuento.valor
        if valor > 0:
            total_descuento = (subtotal * valor)/100

        total_general = (
            total_general['total__sum']+total_otros) - round(total_descuento, 2)
    ...
    if request.method == 'POST':
        ...
        if total_otros == None:
            total_otros = 0
        subtotal = total_base + total_base_reserva+total_excedente+total_consumo_maximo + \
            total_administracion + total_alcantarillado+total_derecho_conexion+total_otros
        valor = cat.descuento.valor
        if valor > 0:
            total_descuento = (subtotal * valor)/100
        ...
        detalle = LecturaDetalle.objects.filter(
            estado=True, catastro=catastro_id).order_by('-id')
        enc = Recaudacion.objects.filter(pk=recaudacion_id).first()
        ...
        for deta in detalle:
            ...
            if det:
                ...
                multa_detalle = MultaDetalle.objects.filter(
                    catastro=catastro_id, estado=True, )
                recaudacion_detalle = RecaudacionDetalle.objects.filter(
                    recaudacion=recaudacion_id, lectura_detalle=lectura_detalle_id)
                for rd in recaudacion_detalle:
                    for md in multa_detalle:
                        if rd.lectura_detalle == md.lectura_detalle:
                            print("Catastro RecaudacionDetalle",
                                  rd.catastro, "Catastro Mulkta", md.catastro)
                            recaudacion_det = RecaudacionDetalle.objects.filter(
                                recaudacion=recaudacion_id, lectura_detalle=lectura_detalle_id).first()
                            multad = MultaDetalle.objects.filter(
                                pk=md.id).first()
                            multa = Multa.objects.filter(
                                pk=md.multa.id).first()
        ...
        return redirect('recaudacion:planilla_recaudada', ultimo)
