...
class Recaudacion(ClaseModelo):
    ...
    abonado = models.ForeignKey(
        "catastro.Abonado", verbose_name="Abonado", on_delete=models.CASCADE)
    descripcion = models.CharField("Descripción", max_length=150)
    pago = models.ForeignKey(
        "parametro.Pago", verbose_name="Pago", on_delete=models.CASCADE)
    descuento = models.ForeignKey(
        "parametro.Descuento", verbose_name="Descuento", on_delete=models.CASCADE)
    total_consumo = models.PositiveIntegerField(
        "Consumo M3", null=True, blank=True)
    ...
    total_excedente = models.DecimalField(
        "Total excedente", max_digits=10, decimal_places=2, null=True, blank=True)
    ...
    subtotal = models.DecimalField(
        "Subtotal", max_digits=10, decimal_places=2, null=True, blank=True)
    total_descuento = models.DecimalField(
        "T. descuento", max_digits=10, decimal_places=2, null=True, blank=True)
...
class RecaudacionDetalle(ClaseModelo):
    ...
    recaudacion = models.ForeignKey(
        "Recaudacion", verbose_name="Recaudación", on_delete=models.CASCADE)
    lectura_detalle = models.ForeignKey(
        "catastro.LecturaDetalle", verbose_name="Lectura detalle", on_delete=models.CASCADE)
...
class RecaudacionMultaDetalle(ClaseModelo):
    ...
    recaudacion_detalle = models.ForeignKey(
        RecaudacionDetalle, verbose_name="Recaudacion detalle", on_delete=models.CASCADE)
    multa_detalle = models.ForeignKey(
        MultaDetalle, verbose_name="Multa detalle", on_delete=models.CASCADE)
    catastro = models.IntegerField("Catastro")
